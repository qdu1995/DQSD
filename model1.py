...
def model(img_width, img_height):
    ...
    ###############################################################################
    input_depth = Input((img_height, img_width, 1))
    # Block a
    ...
    uppoolq1 = UpSampling2D(size=(4, 4), data_format=K.image_data_format())(uppoolq)
    outq = Conv2D(1, (3, 3), padding='same', name='outq')(uppoolq1)
    ...
    uppoolr1 = UpSampling2D(size=(2, 2), data_format=K.image_data_format())(uppoolr)
    ...
    condepth = concatenate([outt, outs, outr, outq])
    W = Conv2D(1, (3, 3), padding='same', name='W', activation='sigmoid')(condepth)
    ###############################################################################
    ...
    ###########################################################################
    conv_m1 = Conv2D(512, (3, 3), padding='same')(fuse2)  # name='conv2d_1'
    ...
    fuse3 = concatenate([fuse3, uppool2])  ###256+256   256+512
    ###########################################################################
    ...
    # outputs=Conv2D(1, (3, 3),padding='same')(conv10_2)
    # conv10_3 = Conv2D(64, (3, 3),   padding='same',dilation_rate=(3, 3),  name='block_10_3')(conv10_2)
    # conv10_3 = BatchNormalization()(conv10_3)
    # conv10_3 = Activation('relu')(conv10_3)
    # conv10_4 = Conv2D(64, (3, 3),   padding='same',dilation_rate=(5, 5),  name='block_10_4')(conv10_2)
    # conv10_4 = BatchNormalization()(conv10_4)
    # conv10_4 = Activation('relu')(conv10_4)
    # conv10_5 = Conv2D(64, (3, 3),   padding='same',dilation_rate=(7, 7),  name='block_10_5')(conv10_2)
    # conv10_5 = BatchNormalization()(conv10_5)
    # conv10_5 = Activation('relu')(conv10_5)###56*5
    ###############################################################################
    w28 = tf.image.resize_images(W, [28, 28])
    weight1 = Lambda(lambda x: x * w28)(convg_4)
    weight2 = Lambda(lambda x: x * (1.0 - w28))(conv7_3)
    ADD1 = Add()([weight1, weight2])  ###28

    w56 = tf.image.resize_images(W, [56, 56])
    weight3 = Lambda(lambda x: x * w56)(convh_3)
    weight4 = Lambda(lambda x: x * (1.0 - w56))(conv8_2)
    ADD2 = Add()([weight3, weight4])  ####56

    w112 = tf.image.resize_images(W, [112, 112])
    weight5 = Lambda(lambda x: x * w112)(convi_3)
    weight6 = Lambda(lambda x: x * (1.0 - w112))(conv9_2)
    ADD3 = Add()([weight5, weight6])  ###112
    ...
    upweight9 = UpSampling2D(size=(2, 2), data_format=K.image_data_format())(weight9)  # ,name='uppoolADD2'
    ...
    upweight10 = UpSampling2D(size=(2, 2), data_format=K.image_data_format())(weight10)  # ,name='uppoolADD3'
    ...
    weight11 = Lambda(lambda x: x * output_fuse8)(conv_fuse8)
    weight11 = Add()([weight11, conv_fuse8])

    conv_11 = Conv2D(128, (3, 3), padding='same', name='block_11')(weight11)
    conv_11 = BatchNormalization()(conv_11)  ##name='batch_11'
